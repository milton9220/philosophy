...
function philosopy_assets() {
    ...
    //wp_enqueue_style("tiny-slider-css",get_theme_file_uri("/assets/css/tiny-slider.css"),null,"1.0");
    ...
    //wp_enqueue_script("tiny-slider-js",get_theme_file_uri("/assets/js/tiny-slider.js"),null,"1.0",true);
...
function philosophy_admin_assets( $hook ) {
    ...
    wp_enqueue_style("admin-style",get_theme_file_uri('/assets/css/admin-style.css'),null,VERSION);
...
function philosophy_cpt_slug_fix( $post_link, $post_id ) {
    ...
    if ( is_object( $p ) && 'book' == get_post_type( $post_id ) ) {
        $genric = wp_get_post_terms( $p->ID, 'genre' );

        if ( is_array( $genric ) && count( $genric ) > 0 ) {
            $slug = $genric[0]->slug;
...
function philosophy_footer_tag_heading_show( $title ) {
...
add_filter( "philosophy_footer_tag_heading", "philosophy_footer_tag_heading_show" );

function philosophy_footer_languages_terms( $tags ) {
...
add_filter( "philosophy_tag_list", "philosophy_footer_languages_terms" );
